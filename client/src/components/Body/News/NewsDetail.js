...
const NewsDetail = () => {
  ...
  const articleComments = comments.filter(
    (comment) => comment.idArticle == articleId
  );
  const commentListRendered = articleComments.map((cmt, index) => (
    <div className="news-detail-comment-container" key={index}>
      <div className="news-detail-comment-left">
        <img
          src={
            "https://secure.gravatar.com/avatar/93048951a869d36af5ed25b84bfe0a90?s=150&r=g&d=https://actoolsource.com/wp-content/plugins/userswp/assets/images/no_profile.png"
          }
          alt="Avatar"
      ...
      <div className="news-detail-comment-actions">
        <button className="news-detail-comment-action">
          <BsFlagFill className="button-icon-comment" />
        </button>
      </div>
    </div>
  ));
  ...
  return (
    <main className="news-page news-grand-container">
      ...
      <div className="news-detail-more-actions">
        ...
        >
          Bình luận <BsFillChatLeftTextFill className="button-icon" />
        </button>
        <button className="news-button">
          Chia sẻ <BsFillShareFill className="button-icon" />
        </button>
        <button className="news-button" onClick={() => navigate("/news")}>
          Tin tức khác <BsNewspaper className="button-icon" />
        </button>
        <button className="news-button">
          Báo cáo <BsFlagFill className="button-icon" />
        </button>
      </div>
