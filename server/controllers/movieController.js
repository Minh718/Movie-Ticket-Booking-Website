    ...
    bookingTicket : async (req,res) => {
        ...
        try{
            ...
            if (idHour.length > 0){
                ...
                if (ticket.length == 0) {
                    await connection.query(`INSERT INTO tbl_ticket (idUser, idHour) VALUES (${idUser}, ${idH})`)
                    const idTicket = await connection.query(`SELECT idTicket from tbl_ticket where idHour = ${idH} AND idUser = ${idUser}`)
                    chairList.forEach( async (chair) => {
                        await connection.query(`INSERT INTO tbl_chair (idTicket, chair) VALUES (${idTicket[0].idTicket}, '${chair}')`)
                    });
                    res.status(200).json("Add ticket success");
                }else{
                    const idTicket = ticket[0].idTicket;
                    chairList.forEach( async (chair) => {
                        await connection.query(`INSERT INTO tbl_chair (idTicket, chair) VALUES (${idTicket}, '${chair}')`)
                    });
